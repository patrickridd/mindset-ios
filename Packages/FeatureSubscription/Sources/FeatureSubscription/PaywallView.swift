    ...
    private var content: some View {
        VStack(spacing: MindsetLayout.spacing25) {
            ...
            VStack(alignment: .leading, spacing: MindsetLayout.spacing20) {
                featureRow(
                    icon: "sparkles", title: "Daily Identity Archetypes",
                    sub: "Personalized insights based on your ritual.")
                featureRow(
                    icon: "clock.arrow.2.circlepath", title: "The Yesterday Bridge",
                    sub: "Connect today's goals with yesterday's wins.")
                featureRow(
                    icon: "cloud.fill", title: "Cross-Platform Sync",
                    sub: "Access your journey on iOS and Android.")
            }
            .padding()
            .background(
                RoundedRectangle(cornerRadius: MindsetLayout.radiusCardLarge).fill(
